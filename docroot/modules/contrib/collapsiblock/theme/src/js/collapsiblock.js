...
((Drupal, cookies, once) => {
  ...
  Drupal.behaviors.collapsiblock = {
    attach: (context, settings) => {
      ...
      const darkMode = settings.collapsiblock.switcher_enabled;
      const themeClass = settings.collapsiblock.switcher_class;
      ...
      titleElements.forEach((titleElement) => {
        ...
        if (darkMode && themeClass) {
          const classInHTML =
            document.documentElement.classList.contains(themeClass);
          const classInBody = document.body.classList.contains(themeClass);
          if (classInHTML || classInBody) {
            collapseButton.classList.add('collapsiblock-color-switcher');
          } else {
            collapseButton.classList.remove('collapsiblock-color-switcher');
          }
        }
        ...
        if (
          ...
          const numberActiveMenuItems = targetElement.querySelectorAll(
            'a.is-active:not(.pager__link)',
          ).length;
          if (
